...
void ADC_DMA_init(ADC_TypeDef* ADCx, uint16_t* RX_Buffer)
{
	...
	/* Configure ADC1 stream*/
	  ...
	  DMA2_Stream0->NDTR = ADC_QUANTITY_CHANNELS;                                     //!< Number of data items to be transferred
	  DMA2_Stream0->PAR = (uint32_t)(&ADCx->DR);                            //!< Peripheral address register
	  DMA2_Stream0->M0AR = (uint32_t)RX_Buffer;           					//!< Peripheral address register
